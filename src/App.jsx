...
const FALLBACK_DECK = [
  {id:"c1",emoji:"🕌",bg:BG_POOL[0]},{id:"c2",emoji:"🌿",bg:BG_POOL[1]},{id:"c3",emoji:"🌊",bg:BG_POOL[2]},
  {id:"c4",emoji:"🏛️",bg:BG_POOL[3]},{id:"c5",emoji:"🌙",bg:BG_POOL[4]},{id:"c6",emoji:"⭐",bg:BG_POOL[5]},
  {id:"c7",emoji:"🏔️",bg:BG_POOL[6]},{id:"c8",emoji:"🌾",bg:BG_POOL[7]},{id:"c9",emoji:"🦋",bg:BG_POOL[0]},
  {id:"c10",emoji:"🌸",bg:BG_POOL[1]},{id:"c11",emoji:"🏜️",bg:BG_POOL[2]},{id:"c12",emoji:"🌋",bg:BG_POOL[3]},
  {id:"c13",emoji:"🐚",bg:BG_POOL[4]},{id:"c14",emoji:"🌀",bg:BG_POOL[5]},{id:"c15",emoji:"🕯️",bg:BG_POOL[6]},
  {id:"c16",emoji:"🗝️",bg:BG_POOL[7]},{id:"c17",emoji:"🌲",bg:BG_POOL[0]},{id:"c18",emoji:"🌌",bg:BG_POOL[1]},
  {id:"c19",emoji:"🏺",bg:BG_POOL[2]},{id:"c20",emoji:"🐉",bg:BG_POOL[3]},{id:"c21",emoji:"🛸",bg:BG_POOL[4]},
  {id:"c22",emoji:"🧿",bg:BG_POOL[5]},{id:"c23",emoji:"🌍",bg:BG_POOL[6]},{id:"c24",emoji:"🎼",bg:BG_POOL[7]},
  {id:"c25",emoji:"💫",bg:BG_POOL[0]},{id:"c26",emoji:"🧲",bg:BG_POOL[1]},{id:"c27",emoji:"📜",bg:BG_POOL[2]},
  {id:"c28",emoji:"🎠",bg:BG_POOL[3]},{id:"c29",emoji:"🌏",bg:BG_POOL[4]},{id:"c30",emoji:"🧊",bg:BG_POOL[5]},
  {id:"c31",emoji:"🌳",bg:BG_POOL[6]},{id:"c32",emoji:"🌕",bg:BG_POOL[7]},{id:"c33",emoji:"🏰",bg:BG_POOL[0]},
  {id:"c34",emoji:"🧸",bg:BG_POOL[1]},{id:"c35",emoji:"💧",bg:BG_POOL[2]},{id:"c36",emoji:"🍀",bg:BG_POOL[3]},
  {id:"c37",emoji:"🎂",bg:BG_POOL[4]},{id:"c38",emoji:"🎨",bg:BG_POOL[5]},{id:"c39",emoji:"🧵",bg:BG_POOL[6]},
  {id:"c40",emoji:"🔑",bg:BG_POOL[7]},{id:"c41",emoji:"🌎",bg:BG_POOL[0]},{id:"c42",emoji:"🧭",bg:BG_POOL[1]},
  {id:"c43",emoji:"📷",bg:BG_POOL[2]},{id:"c44",emoji:"🎶",bg:BG_POOL[3]},{id:"c45",emoji:"🧋",bg:BG_POOL[4]},
  {id:"c46",emoji:"🏖️",bg:BG_POOL[5]},{id:"c47",emoji:"🦢",bg:BG_POOL[6]},{id:"c48",emoji:"👁️",bg:BG_POOL[7]},
];

// ── Shared DB helper: resolve storyteller name from sorted player list ─
function resolveStoryteller(players, storytellerIdx) {
  const sorted = [...players].sort((a, b) => new Date(a.created_at || 0) - new Date(b.created_at || 0));
  return sorted[storytellerIdx % Math.max(sorted.length, 1)]?.name || "";
}
...
function CardFace({ card, size = "large", label = null, highlight = false, voteCount = null }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: radius, flexShrink: 0, background: card?.bg || "#2A1A0A", border, position: "relative", overflow: "hidden", boxShadow: isMini ? "none" : "0 40px 100px rgba(0,0,0,0.7)" }}>
      {card?.image_url && (
        <img src={card.image_url} alt="" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }} onError={e => { e.target.style.display = "none"; e.target.nextSibling.style.display = "flex"; }} />
      )}
      ...
      {label && (
        <div style={{ position: "absolute", bottom: isMini ? 4 : 14, left: 0, right: 0, textAlign: "center", fontSize: isMini ? 8 : 10, letterSpacing: 2, color: "color-mix(in srgb, var(--gold) 45%, transparent)", textTransform: "uppercase", padding: "0 4px" }}>{label}</div>
      )}
      {highlight && (
...
function FaceDown({ size = "large" }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: isMini ? 8 : 20, flexShrink: 0, position: "relative", background: "linear-gradient(145deg,#1A1208,#2D2010)", border: `${isMini ? 1 : 2}px solid rgba(201,149,42,0.2)`, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", boxShadow: isMini ? "none" : "0 40px 100px rgba(0,0,0,0.7)" }}>
...
function SoundToggle() {
  ...
  const toggle = () => { _soundOn = !_soundOn; setOn(_soundOn); if (_soundOn) play("cardPick"); };
  return (
    <button onClick={toggle} title={on ? "Mute sounds" : "Enable sounds"} style={{ background: "transparent", border: "1px solid rgba(201,149,42,0.3)", borderRadius: 8, cursor: "pointer", padding: "6px 10px", fontSize: 16, lineHeight: 1, minWidth: 36, minHeight: 36, color: on ? "#C9952A" : "#7A6E62", transition: "all 0.2s" }}>
      {on ? "🔊" : "🔇"}
    </button>
  );
}
...
function Waiting({ go, S, roomCode, myName, isHost }) {
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel(`wait:${roomCode}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "room_players", filter: `room_code=eq.${roomCode}` }, loadPlayers)
    ...
    <div style={{ flex: 1, padding: "40px 20px" }}>
      <div style={{ maxWidth: 720, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          ...
          <div onClick={() => copyText(roomCode).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2500); })} style={{ display: "inline-flex", alignItems: "center", gap: 16, background: copied ? "color-mix(in srgb, #3A5E2A 30%, transparent)" : "color-mix(in srgb, var(--gold) 8%, transparent)", border: copied ? "2px solid #3A5E2A" : "2px solid color-mix(in srgb, var(--gold) 40%, transparent)", borderRadius: 16, padding: "18px 32px", cursor: "pointer", marginBottom: 16, transition: "all 0.2s", userSelect: "none" }}>
            <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, minWidth: 48 }}>
              <div style={{ width: 36, height: 36, borderRadius: 8, background: copied ? "rgba(58,94,42,0.4)" : "rgba(201,149,42,0.15)", border: `1px solid ${copied ? "#3A5E2A" : "rgba(201,149,42,0.4)"}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, transition: "all 0.2s" }}>{copied ? "✓" : "⎘"}</div>
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap" }}>
            <button onClick={() => copyText(window.location.origin + "?room=" + roomCode).then(() => { setCopiedLink(true); setTimeout(() => setCopiedLink(false), 2500); })} style={{ ...S.btnO, fontSize: 13, padding: "11px 24px", display: "flex", alignItems: "center", gap: 8, borderRadius: 10, background: copiedLink ? "color-mix(in srgb, #3A5E2A 20%, transparent)" : "transparent", border: copiedLink ? "1px solid #3A5E2A" : `1px solid ${S.t.gold}80`, color: copiedLink ? "#7AC87A" : "var(--gold)", transition: "all 0.2s" }}>
              <span>{copiedLink ? "✓ Link Copied!" : "🔗 Copy Invite Link"}</span>
            </button>
          </div>
          <div style={{ fontSize: 11, color: "var(--textMuted)", marginTop: 10 }}>Share the code or the link — friends can join instantly</div>
        </div>
...
function Game({ go, S, roomCode, myName }) {
  ...
  const boardOrderRef = useRef({});

  // ── Helpers ──────────────────────────────────────────────────

  // Build a stable card map from the deck for id→card lookups
  const buildCardMap = () => {
    const map = {};
    deckRef.current.forEach(c => { map[c.id] = c; });
    return map;
  };

  // Hydrate card ids into card objects using the deck
  const hydrateHand = (cardIds) => {
    const map = buildCardMap();
    return cardIds.map(id => map[id]).filter(Boolean);
  };

  // ── loadBoard: fetch plays + votes for the current round and update state ─
  const loadBoard = async (rnd) => {
    const { data: plays } = await supabase.from("card_plays").select("*").eq("room_code", roomCode).eq("round", rnd);
    if (!plays || plays.length === 0) { setBoardCards([]); return; }

    const [{ data: votes }, { data: room }, { data: ps }, { data: cardData }] = await Promise.all([
      supabase.from("votes").select("*").eq("room_code", roomCode).eq("round", rnd),
      supabase.from("rooms").select("storyteller_idx,phase").eq("code", roomCode).single(),
      supabase.from("room_players").select("name,id,created_at").eq("room_code", roomCode).eq("is_active", true),
      supabase.from("cards").select("*").in("id", plays.map(p => p.card_id)),
    ]);

    const currentPhase = room?.phase ?? 0;
    const stName = resolveStoryteller(ps || [], room?.storyteller_idx || 0);

    // Build card map from DB results, falling back to FALLBACK_DECK entries
    const cardMap = {};
    if (cardData) cardData.forEach((c, i) => { cardMap[c.id] = { ...c, bg: c.bg || BG_POOL[i % BG_POOL.length], emoji: c.emoji || "🎴" }; });
    FALLBACK_DECK.forEach(c => { if (!cardMap[c.id]) cardMap[c.id] = c; });

    const cards = plays.map(play => ({
      ...(cardMap[play.card_id] || FALLBACK_DECK[0]),
      owner:          currentPhase >= 3 ? play.player_name : "?",
      isStoryteller:  currentPhase >= 3 ? play.player_name === stName : false,
      isMyCard:       play.player_name === myName,
      votes:          votes ? votes.filter(v => v.voted_card_id === play.card_id).map(v => v.voter_name) : [],
    }));

    // Lock in a stable shuffle order for this round on first sight
    if (!boardOrderRef.current[rnd]) {
      boardOrderRef.current[rnd] = shuffle(cards).map(c => c.id);
    }
    const order = boardOrderRef.current[rnd];
    const cardById = {};
    ...
    boardOrderRef.current[rnd] = ordered.map(c => c.id);
    setBoardCards(ordered);
  };

  // ── checkAndAdvancePhase: single source of truth for automatic phase transitions ─
  // Checks whether a phase threshold has been met and writes the new phase if so.
  // Uses .eq("phase", currentPhase) as a conditional guard so only one client wins
  // even if multiple clients call this concurrently.
  const checkAndAdvancePhase = async (rnd) => {
    const { data: room } = await supabase.from("rooms").select("phase,storyteller_idx").eq("code", roomCode).single();
    if (!room) return;

    const { data: ps } = await supabase.from("room_players").select("name,created_at").eq("room_code", roomCode).eq("is_active", true);
    if (!ps || ps.length === 0) return;

    const stName = resolveStoryteller(ps, room.storyteller_idx);
    const nonStorytellers = ps.filter(p => p.name !== stName);

    if (room.phase === 1) {
      // Phase 1 → 2: all players (including storyteller) have submitted a card
      const { data: plays } = await supabase.from("card_plays").select("id").eq("room_code", roomCode).eq("round", rnd);
      if ((plays?.length || 0) >= ps.length) {
        await supabase.from("rooms").update({ phase: 2 }).eq("code", roomCode).eq("phase", 1);
      }
    } else if (room.phase === 2) {
      // Phase 2 → 3: all non-storyteller players have voted
      const { data: votes } = await supabase.from("votes").select("id").eq("room_code", roomCode).eq("round", rnd);
      if (nonStorytellers.length > 0 && (votes?.length || 0) >= nonStorytellers.length) {
        await supabase.from("rooms").update({ phase: 3 }).eq("code", roomCode).eq("phase", 2);
      }
    }
  };

  // ── dealHands: server-authoritative initial hand dealing ─────
  // Only the earliest-joined player deals to avoid write races.
  // Others poll until their hand appears in dealt_hands.
  const dealHands = async (fullDeck, playerList, room) => {
    let dealtHands = {};
    try { dealtHands = JSON.parse(room?.dealt_hands || "{}"); } catch (e) {}

    // Already have a hand — just restore it
    if (dealtHands[myName]?.length > 0) {
      setHandCards(hydrateHand(dealtHands[myName]));
      return;
    }

    const sorted = [...playerList].sort((a, b) => new Date(a.created_at || 0) - new Date(b.created_at || 0));
    const isDealer = sorted[0]?.name === myName;

    if (!isDealer) {
      // Poll for the dealer to write hands (up to ~10 seconds)
      for (let attempt = 0; attempt < 4; attempt++) {
        await new Promise(r => setTimeout(r, 1500 + attempt * 1000));
        const { data: freshRoom } = await supabase.from("rooms").select("dealt_hands").eq("code", roomCode).single();
        let freshHands = {};
        try { freshHands = JSON.parse(freshRoom?.dealt_hands || "{}"); } catch (e) {}
        if (freshHands[myName]?.length > 0) {
          setHandCards(hydrateHand(freshHands[myName]));
          return;
        }
      }
      // Fallback: if dealer never wrote, deal ourselves to unblock the game
    }

    // Deal 6 cards per player from unused cards
    let usedCards = [];
    try { usedCards = JSON.parse(room?.used_cards || "[]"); } catch (e) {}
    const usedSet = new Set(usedCards);
    let available = fullDeck.filter(c => !usedSet.has(c.id));
    if (available.length < playerList.length * 6) available = [...fullDeck];
    ...
    await supabase.from("rooms").update({ dealt_hands: JSON.stringify(newHands) }).eq("code", roomCode);
    setHandCards(hydrateHand(newHands[myName] || []));
  };
  ...
  useEffect(() => {
    ...
    const init = async () => {
      const [{ data: ps }, { data: room }, { data: cards }] = await Promise.all([
        supabase.from("room_players").select("*").eq("room_code", roomCode).eq("is_active", true),
        supabase.from("rooms").select("*").eq("code", roomCode).single(),
        supabase.from("cards").select("*").eq("active", true),
      ]);

      if (ps) {
        const sorted = [...ps].sort((a, b) => new Date(a.created_at || 0) - new Date(b.created_at || 0));
        setPlayers(sorted);
        setScores(sorted.map(p => ({ name: p.name, score: p.score })));
      }
      if (room) {
        setPhase(room.phase);
        setRound(room.round);
        roundRef.current = room.round;
        setStorytellerIdx(room.storyteller_idx);
      ...
      const fullDeck = (cards && cards.length > 0)
        ? cards.map((c, i) => ({ ...c, bg: c.bg || BG_POOL[i % BG_POOL.length], emoji: c.emoji || "🎴" }))
        : FALLBACK_DECK;
      deckRef.current = fullDeck;

      await dealHands(fullDeck, ps || [], room);
      await loadBoard(room?.round || 1);

      const [{ data: myPlay }, { data: myVote }] = await Promise.all([
        supabase.from("card_plays").select("*").eq("room_code", roomCode).eq("round", room?.round || 1).eq("player_name", myName).single(),
        supabase.from("votes").select("*").eq("room_code", roomCode).eq("round", room?.round || 1).eq("voter_name", myName).single(),
      ]);
    ...
    presenceCh
      .on("presence", { event: "sync" }, () => {
        const online = {};
        Object.keys(presenceCh.presenceState()).forEach(key => { online[key] = true; });
    ...
    const ch = supabase.channel(`game:${roomCode}`)
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "rooms", filter: `code=eq.${roomCode}` }, async (payload) => {
        ...
        setPhase(r.phase);
        setStorytellerIdx(r.storyteller_idx);
        ...
        if (isNewRound) {
          ...
          setClueText("");
          setGameTab("hand");
          // Restore hand from the new dealt_hands
          if (r.dealt_hands && r.dealt_hands !== "{}") {
            try {
              const newHands = JSON.parse(r.dealt_hands);
              if (newHands[myName]?.length > 0) setHandCards(hydrateHand(newHands[myName]));
            } catch (e) {}
          }
        } else {
          // Show round summary overlay when deltas are broadcast
          if (r.round_deltas && r.round_deltas !== "null") {
            try { const d = JSON.parse(r.round_deltas); if (d && typeof d === "object") setRoundDeltas(d); } catch (e) {}
          }
          // Auto-switch to board tab during voting and reveal phases
          if (!r.round_deltas || r.round_deltas === "null") {
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "card_plays", filter: `room_code=eq.${roomCode}` }, async () => {
        await loadBoard(roundRef.current);
        await checkAndAdvancePhase(roundRef.current);
      })
  ...
  const confirmClue = async () => {
    ...
    // Write clue, advance to phase 1, and submit storyteller's card atomically
    await supabase.from("rooms").update({ clue: clueText.trim(), phase: 1 }).eq("code", roomCode);
    const { data: existing } = await supabase.from("card_plays").select("id").eq("room_code", roomCode).eq("round", roundRef.current).eq("player_name", myName).maybeSingle();
  ...
  const submitCard = async () => {
    ...
    play("cardSubmit");
    // Idempotent: check for existing submission first
    const { data: existing } = await supabase.from("card_plays").select("id,card_id").eq("room_code", roomCode).eq("round", roundRef.current).eq("player_name", myName).maybeSingle();
    ...
    setSubmittedCardId(card.id);
    // Phase 1→2 check is handled by the card_plays realtime listener + checkAndAdvancePhase
  };

  const confirmVote = async () => {
    if (boardOverlay === null) return;
    const card = boardCards[boardOverlay];
    if (!card || isStoryteller) return;
    if (card.isMyCard || card.id === submittedCardId) return;
    play("vote");
    // Idempotent: check for existing vote first
    const { data: existing } = await supabase.from("votes").select("id,voted_card_id").eq("room_code", roomCode).eq("round", roundRef.current).eq("voter_name", myName).maybeSingle();
    if (existing) {
      setVotedFor(existing.voted_card_id);
      setVoteConfirmed(true);
      setBoardOverlay(null);
      return;
    }
    const { error: voteErr } = await supabase.from("votes").insert({ room_code: roomCode, round: roundRef.current, voter_name: myName, voted_card_id: card.id });
    if (voteErr) { console.error("vote insert failed", voteErr); return; }
    setVotedFor(card.id);
    setVoteConfirmed(true);
    setBoardOverlay(null);
    // Phase 2→3 check is handled by the votes realtime listener + checkAndAdvancePhase
  };
  ...
  const endRound = async () => {
    ...
    try {
      ...
      const [{ data: plays }, { data: votes }, { data: room }, { data: ps }] = await Promise.all([
        supabase.from("card_plays").select("*").eq("room_code", roomCode).eq("round", rnd),
        supabase.from("votes").select("*").eq("room_code", roomCode).eq("round", rnd),
        supabase.from("rooms").select("storyteller_idx").eq("code", roomCode).single(),
        supabase.from("room_players").select("name,score,created_at").eq("room_code", roomCode).eq("is_active", true),
      ]);
      if (!plays || !room || !ps || ps.length === 0) { setEndRoundLoading(false); return; }

      const sortedPs  = [...ps].sort((a, b) => new Date(a.created_at || 0) - new Date(b.created_at || 0));
      const stName    = sortedPs[room.storyteller_idx % sortedPs.length]?.name || "";
      const nonST     = sortedPs.filter(p => p.name !== stName).map(p => p.name);
      const allVotes  = votes || [];

      const stPlay        = plays.find(p => p.player_name === stName);
      const correctVoters = stPlay ? allVotes.filter(v => v.voted_card_id === stPlay.card_id).map(v => v.voter_name) : [];
      const allOrNone     = correctVoters.length === 0 || correctVoters.length === nonST.length;

      // Dixit scoring
      const deltas = {};
      sortedPs.forEach(p => { deltas[p.name] = 0; });
      if (!allOrNone) {
        deltas[stName] += 3;
        correctVoters.forEach(v => { deltas[v] += 3; });
      } else {
        nonST.forEach(p => { deltas[p] += 2; });
      }
      plays.filter(p => p.player_name !== stName).forEach(p => {
        const vCount = allVotes.filter(v => v.voted_card_id === p.card_id).length;
        if (vCount > 0) deltas[p.player_name] += vCount;
      });

      const newScores = ps.map(p => ({ name: p.name, score: (p.score || 0) + (deltas[p.name] || 0) }));
      await Promise.all(newScores.map(s => supabase.from("room_players").update({ score: s.score }).eq("room_code", roomCode).eq("name", s.name)));

      setRoundDeltas(deltas);
      setScores(newScores);
      play("score");

      // Broadcast deltas so all clients show the round summary overlay
      ...
      if (topScore >= WIN_TARGET) {
        setTimeout(() => play("score"), 400);
        setWinner({ names: newScores.filter(s => s.score === topScore).map(w => w.name), scores: newScores, topScore });
      }
    } catch (err) {
  ...
  const nextRound = async () => {
    ...
    boardOrderRef.current = {};
    setRoundDeltas(null); setSubmittedCardId(null); setVotedFor(null); setVoteConfirmed(false);
    setClueText(""); setConfirmedClue(""); setBoardCards([]); setSelHand(0); setFocusedBoard(0); setGameTab("hand");

    const [{ data: plays }, { data: currentRoom }, { data: ps }] = await Promise.all([
      supabase.from("card_plays").select("*").eq("room_code", roomCode).eq("round", completedRound),
      supabase.from("rooms").select("used_cards,dealt_hands").eq("code", roomCode).single(),
      supabase.from("room_players").select("*").eq("room_code", roomCode).eq("is_active", true),
    ]);
    ...
    // Mark this round's played cards as used
    (plays || []).forEach(p => { if (!usedCards.includes(p.card_id)) usedCards.push(p.card_id); });

    // Build pool of replacement cards: not currently in any hand and not used
    const inAnyHand = new Set(Object.values(dealtHands).flat());
    const usedSet   = new Set(usedCards);
    let available = deckRef.current.filter(c => !usedSet.has(c.id) && !inAnyHand.has(c.id));
    if (available.length < (plays?.length || 0)) available = deckRef.current.filter(c => !inAnyHand.has(c.id));
    if (available.length < (plays?.length || 0)) available = [...deckRef.current];
    const replacements = shuffle(available);
    let replIdx = 0;

    // Replace each player's played card in-slot with a fresh card
    ...
    sortedPs.forEach(p => {
      ...
      const hand      = [...(newDealtHands[p.name] || [])];
      const playedIdx = hand.indexOf(playedCard.card_id);
      if (playedIdx === -1) {
        hand.push(replacements[replIdx++]?.id);
      } else {
        hand[playedIdx] = replacements[replIdx++]?.id || hand[playedIdx];
      }
      newDealtHands[p.name] = hand.filter(Boolean);
    });
    ...
    // Apply new hand locally without waiting for the realtime event
    if (newDealtHands[myName]?.length > 0) setHandCards(hydrateHand(newDealtHands[myName]));
  };
  ...
  const resetGame = async () => {
    roundRef.current = 1;
    boardOrderRef.current = {};
    setWinner(null); setPhase(0); setRound(1); setStorytellerIdx(0);
    setConfirmedClue(""); setClueText(""); setHandCards([]); setBoardCards([]);
    setSubmittedCardId(null); setVoteConfirmed(false); setVotedFor(null);
    setRoundDeltas(null); setSelHand(0); setFocusedBoard(0); setGameTab("hand");
    const reset = players.map(p => ({ ...p, score: 0 }));
    setScores(reset.map(p => ({ name: p.name, score: 0 })));
    await Promise.all(reset.map(p => supabase.from("room_players").update({ score: 0 }).eq("room_code", roomCode).eq("name", p.name)));
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", position: "relative" }}>
      ...
      {gameTab === "hand" && (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflowY: "auto", padding: "20px 16px", gap: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
            <div onClick={() => { if (phase === 0 && isStoryteller) setFocusedHand(selHand); else setFullscreen({ type: "hand", idx: selHand }); }} style={{ cursor: "pointer" }}>
      ...
      {gameTab === "board" && (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflowY: "auto", padding: "20px 16px", gap: 20 }}>
          ...
          {phase === 3 && <div style={{ textAlign: "center", padding: "14px", background: "color-mix(in srgb, var(--gold) 8%, transparent)", border: "1px solid color-mix(in srgb, var(--gold) 25%, transparent)", borderRadius: 12, fontSize: 13, color: "var(--text)" }}>Cards revealed! <span style={{ color: "#C9952A", fontWeight: 600 }}>{STORYTELLER}</span>'s card is highlighted in gold.</div>}

          {boardCards.length === 0 ? (
            <div style={{ textAlign: "center", color: "var(--textMuted)", fontSize: 13, marginTop: 40 }}>No cards on the board yet.</div>
          ) : activeBoardCard && (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
              <div onClick={() => setFullscreen({ type: "board", idx: focusedBoard })} style={{ cursor: "pointer" }}>
                {phase === 1
                  ? <FaceDown size="large" />
                  : <CardFace card={activeBoardCard} size="large" highlight={activeBoardCard.isStoryteller && phase === 3} voteCount={phase === 3 ? activeBoardCard.votes?.length : 0} />
                }
              </div>

              {phase === 3 && (
                <div style={{ textAlign: "center", marginTop: 6, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 14, fontWeight: 700, color: activeBoardCard.isStoryteller ? "#C9952A" : "var(--text)", letterSpacing: activeBoardCard.isStoryteller ? 1 : 0 }}>
              ...
              <div style={{ display: "flex", gap: 8, overflowX: "auto", padding: "4px 0", maxWidth: "100%" }}>
                {boardCards.map((c, i) => (
                  <div key={i} onClick={() => { setFocusedBoard(i); setFullscreen({ type: "board", idx: i }); }} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, cursor: "pointer", flexShrink: 0 }}>
                    <div style={{ transform: focusedBoard === i ? "translateY(-4px)" : "none", transition: "transform 0.2s", outline: c.isStoryteller && phase === 3 ? "3px solid #C9952A" : focusedBoard === i ? "2px solid var(--gold)" : "none", borderRadius: 8, boxShadow: c.isStoryteller && phase === 3 ? "0 0 14px rgba(201,149,42,0.6)" : "none" }}>
                    ...
                    {phase === 3 && (
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2, maxWidth: 64 }}>
                        <span style={{ fontSize: 9, fontWeight: 600, color: c.isStoryteller ? "#C9952A" : "var(--textMuted)", textAlign: "center", maxWidth: 64, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", display: "block" }}>{c.owner || "?"}{c.isStoryteller ? " ★" : ""}</span>
                        {c.votes?.length > 0 ? (
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 1 }}>
                            <span style={{ fontSize: 9, fontWeight: 700, color: "#7AC87A", background: "rgba(30,60,30,0.9)", borderRadius: 6, padding: "1px 5px" }}>{c.votes.length} vote{c.votes.length !== 1 ? "s" : ""}</span>
                            <span style={{ fontSize: 8, color: "var(--textMuted)", textAlign: "center", maxWidth: 64, wordBreak: "break-all", lineHeight: 1.3 }}>{c.votes.join(", ")}</span>
                          </div>
                        ) : <span style={{ fontSize: 8, color: "var(--textMuted)" }}>no votes</span>}
                      </div>
                    )}
                    {votedFor === c.id && phase !== 3 && <span style={{ fontSize: 9, color: "#7AC87A", textAlign: "center" }}>✓</span>}
                  </div>
                ))}
              </div>
      ...
      {boardOverlay !== null && !fullscreen && (
        <div onClick={() => setBoardOverlay(null)} style={{ position: "fixed", inset: 0, background: "var(--overlay)", backdropFilter: "blur(16px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300, padding: 24 }}>
          <div onClick={e => e.stopPropagation()} style={{ maxWidth: 380, width: "100%", display: "flex", flexDirection: "column", alignItems: "center", gap: 20 }}>
            ...
            {phase === 2 && (
              isStoryteller
                ? <div style={{ fontSize: 13, color: "var(--textMuted)", textAlign: "center" }}>You're the Storyteller — you don't vote this round</div>
                : (boardCards[boardOverlay]?.isMyCard || boardCards[boardOverlay]?.id === submittedCardId)
                  ? <div style={{ fontSize: 13, color: "#C4622D", textAlign: "center" }}>This is your card — you can't vote for it</div>
          ...
          {phase === 0 && isStoryteller && (
            <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 380, display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <input
                ...
                onKeyDown={e => { if (e.key === "Enter" && clueText.trim()) { setSelHand(focusedHand); setFocusedHand(null); confirmClue(); } }}
              />
      ...
      {fullscreen && (
        <div onClick={() => setFullscreen(null)} style={{ position: "fixed", inset: 0, background: "var(--fullBg)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 500, cursor: "pointer" }}>
          ...
          {fullscreen.type === "board" ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
              {phase === 1 ? (
                <div style={{ width: "min(68vw, calc(100vh * 0.714))", height: "min(calc(68vw * 1.4), 100vh)", borderRadius: 24, overflow: "hidden", background: "linear-gradient(145deg,#1A1208,#2D2010)", border: "2px solid rgba(201,149,42,0.15)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 0 140px rgba(0,0,0,0.95)", position: "relative" }}>
                ...
                const fc = boardCards[fullscreen.idx];
                return (
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
                    <div style={{ width: "min(68vw, calc(100vh * 0.714))", height: "min(calc(68vw * 1.4), 100vh)", borderRadius: 24, overflow: "hidden", background: fc?.bg, border: fc?.isStoryteller && phase === 3 ? "4px solid #C9952A" : "2px solid rgba(201,149,42,0.2)", boxShadow: fc?.isStoryteller && phase === 3 ? "0 0 140px rgba(0,0,0,0.95),0 0 30px rgba(201,149,42,0.4)" : "0 0 140px rgba(0,0,0,0.95)", position: "relative" }}>
                      {fc?.image_url ? <img src={fc.image_url} alt="" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }} /> : <span style={{ fontSize: "clamp(100px,22vw,220px)", lineHeight: 1, position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)" }}>{fc?.emoji}</span>}
                    </div>
                    {phase === 3 && fc && (
                      <div style={{ textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                        <div style={{ fontSize: 18, fontWeight: 700, color: fc.isStoryteller ? "#C9952A" : "var(--text)" }}>{fc.owner || "?"}{fc.isStoryteller ? " ★" : ""}</div>
                        {fc.votes?.length > 0 ? (
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                            <div style={{ fontSize: 13, fontWeight: 700, color: "#7AC87A" }}>{fc.votes.length} vote{fc.votes.length !== 1 ? "s" : ""}</div>
                            <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 6, maxWidth: 280 }}>
                              {fc.votes.map((voter, vi) => <span key={vi} style={{ fontSize: 12, color: "var(--textMuted)", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 20, padding: "3px 10px" }}>{voter}</span>)}
                            </div>
                          </div>
            ...
            <div style={{ width: "min(68vw, calc(100vh * 0.714))", height: "min(calc(68vw * 1.4), 100vh)", borderRadius: 24, overflow: "hidden", background: handCards[fullscreen.idx]?.bg, border: "2px solid rgba(201,149,42,0.2)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 0 140px rgba(0,0,0,0.95)", position: "relative" }}>
      ...
      <div style={{ flexShrink: 0, borderTop: "1px solid color-mix(in srgb, var(--gold) 8%, transparent)", background: "var(--headerBg)", padding: "6px 16px", display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
        ...
        {scores.map(p => (
          <div key={p.name} style={{ display: "flex", alignItems: "center", gap: 5 }}>
            <div style={{ width: 7, height: 7, borderRadius: "50%", background: onlinePlayers[p.name] ? "#4CAF50" : "#C4622D", boxShadow: onlinePlayers[p.name] ? "0 0 6px #4CAF5088" : "none", flexShrink: 0, transition: "all 0.5s" }} />
            <span style={{ fontSize: 11, color: onlinePlayers[p.name] ? "var(--text)" : "var(--textMuted)", fontWeight: p.name === myName ? 600 : 400 }}>{p.name}{p.name === myName ? " (you)" : ""}</span>
          </div>
        ))}
      </div>
    </div>
